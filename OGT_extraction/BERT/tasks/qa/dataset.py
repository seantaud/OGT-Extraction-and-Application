...
from BERT.tasks.qa.utils_qa import postprocess_qa_predictions
from OGT_QA_datasets import load_samples, load_examples, process_examples


class SQuAD:

    def __init__(self, tokenizer: AutoTokenizer, data_args, training_args, qa_args,ckpt_dir) -> None:
        self.data_args = data_args
        self.training_args = training_args
        self.qa_args = qa_args
        self.version_2 = True
        self.sentence_sampling = qa_args.sentence_sampling
            
        self.tokenizer = tokenizer
        self.max_seq_len = 384
        raw_datasets = load_examples()
        ...
        if training_args.do_train:
            ...
            f"Preprocessing {len(raw_datasets['train'])} example predictions split into {len(self.train_dataset)} features."
        ...
        if training_args.do_eval:
            ...
            f"Preprocessing {len(raw_datasets['validation'])} example predictions split into {len(self.eval_dataset)} features."
            if data_args.max_eval_samples is not None:
                self.eval_dataset = self.eval_dataset.select(range(data_args.max_eval_samples))

        self.predict_dataset = None
        self.predict_examples = raw_datasets["test"]
        with training_args.main_process_first(desc="prediction dataset map pre-processing"):
            ...
            f"Preprocessing {len(raw_datasets['test'])} example predictions split into {len(self.predict_dataset)} features."
            

        self.data_collator = default_data_collator

        self.metric = evaluate.load("./metric/squad_v2")
